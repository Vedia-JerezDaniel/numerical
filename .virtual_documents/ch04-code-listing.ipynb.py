...
fig = plt.figure(figsize=(8, 2.5), facecolor="#f3f3f3")
...
# fig.savefig("graph.png", dpi=100, facecolor="#f2f2f2")
# fig.savefig("graph.pdf", dpi=300, facecolor="#f2f2f2")
...
for n in range(4):
    ...
    axes[n].set_title("legend(loc=get_ipython().run_line_magic("d)"", " % (n+1))")
...
for n in range(1, 9):
    ax.plot(x, n * x, label="y(x) = get_ipython().run_line_magic("d*x"", " % n)")
...
for m in range(nrows):
    for n in range(ncols):
        axes[m, n].set_xticks([])
        axes[m, n].set_yticks([])
        axes[m, n].text(0.5, 0.5, "axes[get_ipython().run_line_magic("d,", " %d]\" % (m, n),")
...
[ax.text(0.5, 0.5, "axget_ipython().run_line_magic("d"", " % n, horizontalalignment='center') for n, ax in enumerate(axes)]")
